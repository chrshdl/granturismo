...
class Decrypter(object):
  def __init__(self, gt: GT_Version = GT_Version.GT7):
    if gt == GT_Version.GT6:
      ...
      self._KEY = b'Simulator Interface Packet GT7 v' #er 0.0 ... we only need 32-bits
      self._BYTE_ORDER = 'little'
      self._IV_MASK = 0xDEADBEAF
      self._GT_ID = 0x47375330
    else: 
      raise NotImplementedError("GTA versions other than GT6 and GT7 are not implemented")
  ...
  def decrypt(self, buffer: bytearray) -> bytearray:
    ...
    iv = bytearray()
    iv.extend(iv2.to_bytes(4, self._BYTE_ORDER))
    iv.extend(iv1.to_bytes(4, self._BYTE_ORDER))
